...
// // ✅ FIXED: Now safely handles missing from/to query parameters
// const getAppointments = async (req, res) => {
//   try {
//     // Safely extract query params – they may be undefined
...
//     // ✅ Apply date range filter ONLY if both from and to are provided and non‑empty
//     if (from && to) {
//       // Assuming we filter on the 'date' column (format YYYY-MM-DD)
//       q = q.gte('date', from).lte('date', to);
//     }
//     // If from or to is missing, we simply don't filter by date.
...
//   sendAppointmentRescheduled,
//   sendAppointmentCancelled,
//   sendAppointmentFeedbackInvitation   // NEW
// } = require('../services/emailService');
// const {
//   getBookedSlotsForDate,
//   createCalendarEvent,
...
//     appointmentType: appointmentType || 'Consult',
//     status: 'Pending',
//     appointment_number: appointmentNumber   // NEW
//   };
...
//       description: data.reason,
//       email: req.user.email,
//       appointmentNumber: data.appointment_number   // NEW
//     }).catch((e) => console.error('[appointments] confirmation email failed:', e));
//   }
...
//   // ─── If completed, send feedback invitation ────────────────
//   if (status === 'Completed') {
...
//     // If new status is 'Completed', send feedback invitation
...
// //   }
// //    console.log(`🔍 Looking for appointment number: ${number} (type: ${typeof number})`);

// //   const { data, error } = await supabase
// //     .from(T)
// //     .select('id, patientName, patientPhone, email, petName, date, time, hospital, appointment_type, reason, status')
// //     .eq('appointment_number', String(number))
// //     .maybeSingle();

// //   if (error || !data) {
// //     return res.status(404).json({ message: `Appointment not found for number: ${number}` });

// //   }

// //   // Optional validation: if email/phone provided, verify they match
// //   if (email || phone) {
...
//     description: data.reason,
//     source: 'dashboard',
//     appointmentNumber: data.appointment_number   // ✅ FIXED
//   }).catch((e) => console.error('[appointments] superadmin new-booking email failed:', e));
...
//         status: patch.status,
//         message: req.body.message || undefined,
//         appointmentNumber: appt.appointment_number   // ✅ Fixed: use appt, not existing
//       }).catch((e) => console.error('[appointments] status update email failed:', e));
//     }
