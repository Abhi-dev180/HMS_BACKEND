...
// ─── Helper: generate allowed slots (9:00 - 17:00, 30min steps) ──
const generateAllowedSlots = () => {
  const slots = [];
  for (let h = 9; h < 17; h++) {
...
const ALLOWED_SLOTS = generateAllowedSlots();
...
const bookAppointment = async (req, res) => {
  ...
  if (!ALLOWED_SLOTS.includes(time)) {
    return res.status(400).json({ message: 'Selected time is outside business hours (9:00 AM – 5:00 PM).' });
  }
