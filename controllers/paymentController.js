...
const stripeSvc = require("../services/stripeService");
const { PLANS } = require("../config/stripePlans");
const { sendInvoicePaidEmail } = require("../services/emailService");
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `For ${booking.contact_name || "Guest"}`,
            },
            unit_amount: amount,
          },
          quantity: 1,
...
const verifySession = async (req, res) => {
  ...
  try {
    ...
    if (paid && isSupabaseConfigured()) {
      ...
      const demoBookingId = session.metadata?.demo_booking_id;
  ...
  switch (event.type) {
    ...
    case "invoice.paid": {
      ...
      if (isSupabaseConfigured()) {
        ...
        // Send invoice email if we have either booking or user email
        if (booking || userEmail) {
          ...
          // ✅ Retrieve invoice WITHOUT expand – invoice_pdf is included by default
          ...
          await sendInvoicePaidEmail({
            ...
            console.error("[webhook] Payment invoice email failed:", e),
          );
