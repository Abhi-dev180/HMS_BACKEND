...
//         // Annotate the user row for quick access to plan details
//         try {
...
//           });
//         } catch (e) {
//           console.error('[payments] annotate user failed:', e);
//         }
...
//               userId: session.metadata.user_id,
//               planKey: session.metadata.plan_key,
//               startDate: saved?.start_date || null,
//               expiryDate: saved?.expiry_date || null,
//               status: 'active'
//             });
//           } catch (e) {
//             console.error('[payments] annotate user failed:', e);
//           }
//         } catch (e) {
//           console.error('[payments] save one-time subscription failed:', e);
//         }
//       }
//     }

//     // If Stripe session contains a subscription id, try to fetch the saved
//     // subscription record from the DB so the frontend can show start/end dates.
...
//             .limit(1)
//             .maybeSingle();
//           if (!subErr && subData) subscriptionRecord = subData;
//         }

//         if (!subscriptionRecord && sessionId) {
//           const { data: payData, error: payErr } = await supabase
//             .from('payments')
//             .select('subscription_id, stripe_session_id')
//             .eq('stripe_session_id', sessionId)
//             .limit(1)
//             .maybeSingle();
//           if (!payErr && payData?.subscription_id) {
//             const { data: subData, error: subErr } = await supabase
//               .from('subscriptions')
//               .select('*')
...
//             if (!subErr && subData) subscriptionRecord = subData;
//           }
//           if (!subscriptionRecord && !session.subscription && payData?.stripe_session_id) {
//             const fallbackKey = `one_time_${payData.stripe_session_id}`;
//             const { data: subData, error: subErr } = await supabase
//               .from('subscriptions')
//               .select('*')
...
//       }
//     } catch (e) {
//       console.error('[payments] fetch subscription record failed:', e);
//     }

//     if (!subscriptionRecord && session.mode === 'payment' && session.metadata?.plan_key && paid && isSupabaseConfigured()) {
//       try {
...
//           await supabase.from('payments').update({ subscription_id: created.id, updated_at: new Date().toISOString() }).eq('stripe_session_id', session.id);
//         }
//       } catch (e) {
//         console.error('[payments] ensure one-time subscription failed:', e);
//       }
//     }
...
//   // Helper to compute expiry date from a start date and plan metadata
...
//   // Ensure we have start/expiry; if not provided, compute from now using plan metadata
...
// // When saving a subscription, also annotate the linked user row so the
// // frontend and registration flows can easily access plan start/end and status.
// const annotateUserWithSubscription = async ({ userId, planKey, startDate, expiryDate, status }) => {
//   if (!userId) return null;
//   try {
//     const patch = {
//       plan_key: planKey || null,
//       plan_start: startDate || null,
//       plan_end: expiryDate || null,
...
//         // Ensure payments row stores the plan_key from session metadata (one-time or subscription)
//         try {
//           const planKeyMeta = session.metadata?.plan_key || null;
//           if (planKeyMeta) {
//             await supabase
//               .from('payments')
//               .update({ plan_key: planKeyMeta, updated_at: new Date().toISOString() })
//               .eq('stripe_session_id', session.id);
//           }
//         } catch (e) {
//           console.error('[webhook] update payment plan_key failed:', e);
//         }
...
//             currency: subscription.items.data[0]?.price?.currency || 'usd'
//           });
//           // Also link the payments row for this checkout session to the created subscription
//           try {
...
//               await supabase
//                 .from('payments')
//                 .update({ subscription_id: savedSub.id, updated_at: new Date().toISOString() })
//                 .eq('stripe_session_id', session.id);
//             }
//           } catch (e) {
//             console.error('[webhook] linking payment to subscription failed:', e);
//           }
//         }
//         // Handle one-time payments that still represent a plan purchase
//         else if (session.mode === 'payment' && session.metadata?.plan_key) {
//           try {
//             const planKey = session.metadata.plan_key;
//             // create a subscription-like record for one-time purchases so we can track start/end
//             const saved = await saveSubscription({
//               userId: session.metadata.user_id,
//               hospitalId: session.metadata.hospital_id,
//               planKey,
...
//               stripeCustomerId: session.customer || null,
//               status: 'active',
//               // startDate / expiryDate will be computed inside saveSubscription if omitted
//               amount: session.amount_total || 0,
//               currency: session.currency || 'usd'
//             });
//             if (saved && saved.id) {
//               await supabase.from('payments').update({ subscription_id: saved.id, updated_at: new Date().toISOString() }).eq('stripe_session_id', session.id);
//             }
//           } catch (e) {
//             console.error('[webhook] one-time saveSubscription failed:', e);
//           }
//         }
//       }
//       break;
...
// // Admin helper to re-sync a Stripe session into DB (payments / subscriptions).
...
//     }).eq('stripe_session_id', session.id);
//   } catch (e) {
//     console.error('[payments] sync update payment failed:', e);
//   }

//   // If subscription, ensure subscription saved and payments linked
//   if (session.subscription) {
//     const subscription = await stripeSvc.retrieveSubscription(session.subscription);
//     const saved = await saveSubscription({
//       userId: session.metadata?.user_id,
...
//         await supabase.from('payments').update({ subscription_id: saved.id, updated_at: new Date().toISOString() }).eq('stripe_session_id', session.id);
//       } catch (e) {
//         console.error('[payments] sync link failed:', e);
//       }
//     }
//     // annotate user
//     try {
...
//   }
//   else if (session.mode === 'payment' && session.metadata?.plan_key) {
//     // create a subscription-like record for one-time purchases
//     try {
//       const planKey = session.metadata.plan_key;
//       const saved = await saveSubscription({
//         userId: session.metadata?.user_id,
...
//       }
//     } catch (e) {
//       console.error('[payments] sync one-time failed:', e);
//     }
//   }
...
// module.exports.syncSession = syncSession;
