...
https://cal.com/booking/${bookingUid || ""}?changes=true
    ...
    // ✅ Create event with Google Meet
    // Removed the erroneous "location" key because meetUrl isn't available yet.
    const eventRes = await fetch(
      "https://www.googleapis.com/calendar/v3/calendars/primary/events?conferenceDataVersion=1",
      {
        method: "POST",
...
const createBooking = async ({ start, name, email, bookingDetails = {} }) => {
  ...
  // Sanitize email to remove trailing comma
  const cleanEmail = String(email).replace(/,\s*$/, "").trim();
  ...
  // ✅ Strategy 1: Poll Cal.com for meeting URL (Increased timeout to 4s per attempt)
  if (bookingUid) {
    console.log("[calcomService] 🔄 Polling Cal.com for meeting URL...");

    // Changed from 5 attempts to 6, and timeout from 2000ms to 4000ms
  ...
  // ✅ Strategy 2: Create Google Meet directly (Fallback)
  if (!meetingUrl) {
    console.log(
      "[calcomService] 🎥 Creating Google Meet directly with booking details...",
    );
