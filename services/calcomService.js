...
//  * Create Google Meet event using Google Calendar API with full booking details
//  */
// const createGoogleMeetEvent = async (startTimeIso, name, email, bookingDetails = {}, bookingUid = null) => {
//   // Use calendar-specific environment variables
//   const clientId = process.env.GOOGLE_CLIENT_ID;
//   const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
//   const refreshToken = process.env.GOOGLE_CALENDAR_REFRESH_TOKEN || process.env.GOOGLE_REFRESH_TOKEN;
...
//         summary: `Demo Meeting: ${hospitalName || 'Hospital Demo'} with ${contactName || name || 'Guest'}`,
//         description: description.trim(),
//         location: meetUrl || 'Virtual Meeting Link',
//         start: { dateTime: start.toISOString(), timeZone: TZ },
//         end: { dateTime: end.toISOString(), timeZone: TZ },
//         attendees: email ? [{ email, responseStatus: 'needsAction' }] : [],
//         conferenceData: {
...
//   // ✅ Sanitize email to remove trailing comma which caused api_validation_error
//   const cleanEmail = String(email).replace(/,\s*$/, '').trim();
...
//       attendee: {
//         name: name || 'Guest',
//         email: cleanEmail, // <-- Using sanitized email
//         timeZone: TZ,
//         language: 'en'
//       },
//       // ✅ Include booking metadata for Cal.com
...
//   // ✅ Strategy 2: Create Google Meet directly with all booking details (Pass bookingUid)
//   if (!meetingUrl) {
//     console.log('[calcomService] 🎥 Creating Google Meet directly with booking details...');
//     const meetUrl = await createGoogleMeetEvent(start, name, email, bookingDetails, bookingUid);
...
//   // ──────────────────────────────────────────────────────────────
//   // ✅ FIX: Validate URL and PRESERVE Zoom links instead of overwriting them
//   // ──────────────────────────────────────────────────────────────
//   if (meetingUrl) {
//     // Check if it's a valid Google Meet link
//     const isGoogleMeet = meetingUrl.match(/meet\.google\.com\/([a-z0-9]{3}-[a-z0-9]{4}-[a-z0-9]{3})/i);
//     // Check if it's a valid Zoom link
//     const isZoom = meetingUrl.includes('zoom.us');

//     if (isGoogleMeet) {
//       // Ensure Google Meet code is lowercase
//       const code = isGoogleMeet[1].toLowerCase();
//       meetingUrl = `https://meet.google.com/${code}`;
//     } else if (isZoom) {
//       // ✅ EXACTLY WHAT YOU NEED: Do NOT overwrite Zoom!
//       console.log('[calcomService] ✅ Preserving Zoom link from Cal.com:', meetingUrl);
//     } else {
//       // If it's some other invalid format (unlikely), fallback to Google Meet
//       console.warn('[calcomService] ⚠️ URL is not a valid Google Meet or Zoom link. Generating fallback...');
//       const seed = bookingUid || `${Date.now()}-${Math.random().toString(36)}`;
//       const meetCode = generateGoogleMeetCode(seed);
//       meetingUrl = `https://meet.google.com/${meetCode}`;
//     }
...
const getSlotGrid = async () => {
  ...
  const offset =
    firstDay && firstDay[0]
      ? firstDay[0].start.slice(-6)
      : process.env.CALCOM_UTC_OFFSET || "+00:00";
  const nowMs = Date.now();

  const grid = [];
  for (const date of Object.keys(data)) {
    const availSet = new Set(
      data[date].map((s) => new Date(s.start).getTime()),
    );
    for (let h = START_HOUR; h < END_HOUR; h += 1) {
      for (let m = 0; m < 60; m += STEP_MIN) {
        const iso = `${date}T${pad(h)}:${pad(m)}:00.000${offset}`;
...
const createGoogleMeetEvent = async (
  ...
  console.log(
    "[calcomService] 🔍 Creating Google Meet with booking details:",
    bookingDetails,
  );

  if (!clientId || !clientSecret || !refreshToken) {
    console.warn(
      "[calcomService] ❌ Missing Google OAuth2 credentials for Meet creation.",
    );
    return null;
  }
...
https://cal.com/booking/${bookingUid || ""}?changes=true
    `.trim();
    ...
    if (meetUrl) {
      ...
      console.warn(
        "[calcomService] ⚠️ No Meet link in response, but event was created.",
      );
      console.warn(
        "[calcomService] Response data:",
    ...
    console.error(
      "[calcomService] ❌ Error creating Google Meet:",
      err.message,
    );
    console.error("[calcomService] Stack:", err.stack);
    return null;
...
const createBooking = async ({ start, name, email, bookingDetails = {} }) => {
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
      ...
      try {
        ...
        if (fetchRes.ok) {
          ...
          // Check references for conference URL
          if (Array.isArray(fetchedData.references)) {
            const ref = fetchedData.references.find(
              (r) =>
                r.url &&
                (r.url.includes("meet.google.com") ||
                  r.url.includes("zoom.us")),
            );
            if (ref?.url) {
              url = ref.url;
            }
          ...
          if (url && !url.includes("integrations:") && url.startsWith("http")) {
            console.log(
              `[calcomService] ✅ Found meeting URL on attempt ${attempt}:`,
              url,
            );
            meetingUrl = url;
            break;
          ...
          console.log(
            `[calcomService] Attempt ${attempt}: No valid URL yet. Waiting...`,
          );
        }
      } catch (fetchErr) {
        console.error(
          `[calcomService] Error polling attempt ${attempt}:`,
          fetchErr.message,
        );
      }
    }
  ...
  if (!meetingUrl) {
    console.log(
      "[calcomService] 🎥 Creating Google Meet directly with booking details...",
    );
    ...
    if (meetUrl) {
      ...
      if (bookingUid) {
        try {
          ...
          console.log(
            "[calcomService] Updated Cal.com booking with Google Meet URL",
          );
        } catch (updateErr) {
          console.warn(
            "[calcomService] Could not update Cal.com booking:",
            updateErr.message,
          );
        }
      }
      ...
      console.log(
        "[calcomService] ✅ Generated fallback Google Meet URL:",
        meetingUrl,
      );
    }
  }
  ...
  if (meetingUrl) {
    // Check if it's a valid Google Meet link
    const isGoogleMeet = meetingUrl.match(
    ...
    if (isGoogleMeet) {
      ...
      // ✅ EXACTLY WHAT YOU NEED: Do NOT overwrite Zoom!
      console.log(
        "[calcomService] ✅ Preserving Zoom link from Cal.com:",
        meetingUrl,
      );
    } else {
      // If it's some other invalid format (unlikely), fallback to Google Meet
      console.warn(
        "[calcomService] ⚠️ URL is not a valid Google Meet or Zoom link. Generating fallback...",
      );
      const seed = bookingUid || `${Date.now()}-${Math.random().toString(36)}`;
      const meetCode = generateGoogleMeetCode(seed);
      meetingUrl = `https://meet.google.com/${meetCode}`;
